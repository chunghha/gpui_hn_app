...
impl AppState {
        ...
        if let Some(api_service) = api_service {
            ...
            if let Some(new_stories) = rx.next().await {
                let _ = entity.update(cx, |state, _cx| {
                    ...
                    // Let's just update loaded_count based on what we processed.
                    // Wait, if we use loaded_count as index into story_ids, we must increment it by the number of IDs processed, regardless of success.
                    // We calculated `end` before. We need that value.
    ...
    pub fn hide_webview(entity: Entity<Self>, cx: &mut App) {
        entity.update(cx, |state, cx| {
            ...
            // Let's modify the ViewMode again.
            // Let's try to be smart in `hide_webview`. We need to restore the story view.
            // But we can't, as the story is gone.
            // The investigator suggested `Webview(String)`. Let's stick to that for a bit.
            // When we hide, where do we go? We can't go back to a story we don't have.
            // So we must go to the List. This is a UX degradation.
            // The alternative is to change ViewMode to: `Story(Story)` and `StoryWithWebview(Story, String)`
            // Or `Webview { story: Story, url: String }`.
            // Let's try another approach. Keep `selected_story` around.
            // Let's see: `select_story` sets the story. If we go to webview, we could keep it.
            // But `view_mode` replaces `selected_story`.
            // What if we bring `selected_story` back?
